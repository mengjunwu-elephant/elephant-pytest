...
co = [[0, 20, 0, -90, 0, 90, 0], [30, 18, 0, -100, -20, 80, 30]]
...
logging.basicConfig(filename='left_block_test_0810.log', level=logging.INFO, format=LOG_FORMAT, datefmt=DATE_FORMAT)
...
def generate_random_angles():
    ...
    print(f"随机生成角度为{random_pos_a}")
    return random_pos_a


def stable_angles():
    for a in an:
        m.send_angles(a, age_sp)
        # time.sleep(5)
        wait_until(a, 0)
...
def random_angle_move():
    random_an = generate_random_angles()
...
if __name__ == '__main__':
    m = Mercury('/dev/left_arm', debug=False)
