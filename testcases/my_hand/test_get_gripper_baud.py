...
# 从 Excel 中提取测试数据
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "get_gripper_baud")
...
@allure.feature("获取夹爪波特率")
@allure.story("获取夹爪通信波特率")
@pytest.mark.parametrize("case", cases, ids=[case["title"] for case in cases])
def test_get_gripper_baud(device, case):
    logger.info(f'》》》》》用例【{case["title"]}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')

    with allure.step("调用 get_gripper_baud 接口"):
        response = device.m.get_gripper_baud()
    ...
    with allure.step("断言返回值等于期望值"):
        ...
        assert response == case['expect_data'], f"波特率不匹配，期望 {case['expect_data']}，实际 {response}"
