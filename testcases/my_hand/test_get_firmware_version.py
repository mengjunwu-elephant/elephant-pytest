...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "get_gripper_firmware_version")
...
def test_get_firmware_version(device, case):
    ...
    with allure.step("断言返回值类型为 float"):
        assert isinstance(response, float), f"返回类型错误，应为 float，实际为 {type(response)}"
