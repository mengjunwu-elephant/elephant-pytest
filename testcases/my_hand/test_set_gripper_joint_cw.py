...
@allure.feature("夹爪关节CW值设置")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_gripper_joint_cw(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 set_gripper_joint_cw，joint={case['joint']}，parameter={case['parameter']}"):
        set_res = device.m.set_gripper_joint_cw(case["joint"], case["parameter"])
        get_res = device.m.get_gripper_joint_cw(case["joint"])
        logger.debug(f"set_res: {set_res}, get_res: {get_res}")

    with allure.step("断言返回类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误，期望 int，实际为 {type(set_res)}"

    with allure.step("断言设置与查询结果一致"):
...
def test_set_gripper_joint_cw_exception(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{case["title"]}】开始测试《《《《《')
    with allure.step("打印测试参数信息"):
        logger.debug(f'test_api:{case["api"]}')
    ...
    with allure.step(f"调用 {case['api']} 异常场景接口，参数 joint={case['joint']}, parameter={case['parameter']}"):
        with pytest.raises(ValueError, match=".*"):
            device.m.set_gripper_joint_cw(case["joint"], case["parameter"])
