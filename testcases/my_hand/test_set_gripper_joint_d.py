...
# 从Excel中提取数据
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "set_gripper_joint_d")
...
def test_set_gripper_joint_d(device, case):
    title = case['title']
    ...
    with allure.step(f"调用接口 set_gripper_joint_D，joint={case['joint']}，parameter={case['parameter']}"):
        set_res = device.m.set_gripper_joint_D(case["joint"], case["parameter"])
        get_res = device.m.get_gripper_joint_D(case["joint"])
...
def test_set_gripper_joint_d_exception(device, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    with allure.step("打印测试参数信息"):
        logger.debug(f'test_api: {case["api"]}')
