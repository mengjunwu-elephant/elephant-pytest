...
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "get_gripper_joint_mini_pressure")
...
def test_get_gripper_joint_mini_pressure_normal(device, case):
    title = case['title']
    joint = case['joint']
    ...
    with allure.step(f"调用接口 get_gripper_joint_mini_pressure({joint})"):
        response = device.m.get_gripper_joint_mini_pressure(joint)
...
def test_get_gripper_joint_mini_pressure_exception(device, case):
    title = case['title']
    joint = case['joint']
    ...
    with allure.step(f"调用 {case['api']} 异常场景接口，参数 joint={case['joint']}"):
        with pytest.raises(ValueError, match=f".*{joint}.*"):
            device.m.get_gripper_joint_mini_pressure(int(joint))
