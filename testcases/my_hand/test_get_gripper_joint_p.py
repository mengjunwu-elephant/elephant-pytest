...
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "get_gripper_joint_p")
...
@allure.feature("获取夹爪P关节值")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_get_gripper_joint_p_normal(device, case):
    title = case['title']
    joint = case['joint']
    ...
    with allure.step(f"调用接口 get_gripper_joint_P({joint})"):
        response = device.m.get_gripper_joint_P(joint)
...
def test_get_gripper_joint_p_exception(device, case):
    title = case['title']
    joint = case['joint']
