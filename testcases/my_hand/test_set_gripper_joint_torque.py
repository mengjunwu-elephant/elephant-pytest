...
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "set_gripper_joint_torque")
...
@allure.feature("夹爪关节扭矩设置")
@allure.story("设置夹爪关节扭矩接口")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_gripper_joint_torque(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口，参数 joint={case['joint']}, parameter={case['parameter']}"):
        ...
        get_res = device.m.get_gripper_joint_torque(case["joint"])
    ...
    with allure.step("断言设置结果与查询结果"):
        allure.attach(str(expected), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(set_res), name="set接口返回", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="get接口返回", attachment_type=allure.attachment_type.TEXT)
        assert set_res == expected, f"用例【{title}】set接口断言失败，期望 {expected}，实际 {set_res}"
        assert get_res == case["parameter"], f"用例【{title}】get接口断言失败，期望 {case['parameter']}，实际 {get_res}"

    logger.info(f'✅ 用例【{title}】测试通过')

@allure.feature("夹爪关节扭矩设置")
@allure.story("设置夹爪关节扭矩接口 - 异常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "exception"], ids=lambda c: c["title"])
def test_set_gripper_joint_torque_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 异常场景接口，参数 joint={case['joint']}, parameter={case['parameter']}"):
        with pytest.raises(ValueError, match=f".*扭矩值为{case['parameter']}.*"):
            device.m.set_gripper_joint_torque(case["joint"], case["parameter"])
