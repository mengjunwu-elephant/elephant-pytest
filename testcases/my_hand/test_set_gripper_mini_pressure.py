...
cases = get_test_data_from_excel(MyHandBase.TEST_DATA_FILE, "set_gripper_joint_mini_pressure")
...
def test_set_gripper_joint_mini_pressure(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口，参数 joint={case['joint']}, parameter={case['parameter']}"):
        set_res = device.m.set_gripper_joint_mini_pressure(case["joint"], case["parameter"])
        get_res = device.m.get_gripper_joint_mini_pressure(case["joint"])
