...
cases = get_test_data_from_excel(MercuryBase.MY_HAND_TEST_DATA_FILE, "set_hand_gripper_clockwise")
...
@allure.feature("设置夹爪顺时针可运行误差")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_hand_gripper_clockwise_normal(device, case):
    logger.info(f"》》》》》用例【{case['title']}】开始测试《《《《《")
    logger.debug(f"test_api:{case['api']}")
    logger.debug(f"test_joint:{case['joint']}")
    logger.debug(f"test_parameter:{case['parameter']}")

    with allure.step("调用接口设置夹爪顺时针可运行误差"):
        set_res = device.ml.set_hand_gripper_clockwise(case["joint"], case["parameter"])
        get_res = device.ml.get_hand_gripper_clockwise(case["joint"])
    ...
    with allure.step("断言获取接口返回的顺时针可运行误差正确"):
...
def test_set_hand_gripper_clockwise_exception(device, case):
    logger.info(f"》》》》》用例【{case['title']}】开始测试《《《《《")
    logger.debug(f"test_api:{case['api']}")
    logger.debug(f"test_parameter:{case['parameter']}")

    with allure.step(f"调用接口，预期抛出 MercuryDataException,关节为{case['joint']},顺时针可运行误差为{case['parameter']}"):
        with pytest.raises(MercuryDataException, match=f".*") as exc:
            device.ml.set_hand_gripper_clockwise(case["joint"], case["parameter"])
