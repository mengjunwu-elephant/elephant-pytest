...
def test_set_hand_gripper_clockwise_normal(device, case):
    ...
    with allure.step("调用接口设置夹爪顺时针可运行误差"):
        set_res = device.mc.set_hand_gripper_clockwise(case["joint"], case["parameter"])
        get_res = device.mc.get_hand_gripper_clockwise(case["joint"])
...
def test_set_hand_gripper_clockwise_exception(device, case):
    ...
    with allure.step(f"调用接口，预期抛出 MercuryDataException,关节为{case['joint']},顺时针可运行误差为{case['parameter']}"):
        with pytest.raises(MercuryDataException, match=f".*"):
            device.mc.set_hand_gripper_clockwise(case["joint"], case["parameter"])
