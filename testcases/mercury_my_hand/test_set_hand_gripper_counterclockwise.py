...
cases = get_test_data_from_excel(MercuryBase.MY_HAND_TEST_DATA_FILE, "set_hand_gripper_counterclockwise")
...
def test_set_hand_gripper_counterclockwise_normal(device, case):
    ...
    with allure.step("调用接口设置夹爪逆时针可运行误差"):
        set_res = device.ml.set_hand_gripper_counterclockwise(case["joint"], case["parameter"])
        get_res = device.ml.get_hand_gripper_counterclockwise(case["joint"])
