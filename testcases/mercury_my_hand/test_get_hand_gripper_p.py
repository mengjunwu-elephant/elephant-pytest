...
# 从Excel中提取测试用例数据
cases = get_test_data_from_excel(MercuryBase.MY_HAND_TEST_DATA_FILE, "get_hand_gripper_p")
...
@allure.feature("获取夹爪P参数")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_get_hand_gripper_p_normal(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_joint: {case['joint']}")

    with allure.step("调用 get_hand_gripper_p"):
        response = device.ml.get_hand_gripper_p(case["joint"])
...
def test_get_hand_gripper_p_exception(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_joint: {case['joint']}")
    ...
    with allure.step(f"校验非法参数是否抛出 MercuryDataException,关节为{case['joint']}"):
        with pytest.raises(MercuryDataException) as exc:
            device.ml.get_hand_gripper_p(int(case['joint']))

    logger.info(f"✅ 异常断言成功，用例【{case['title']}】测试通过,异常信息：{exc.value}")
    logger.info(f"》》》用例【{case['title']}】测试完成《《《")
