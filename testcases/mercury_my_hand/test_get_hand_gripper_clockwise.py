...
@allure.feature("获取单关节顺时针可运行误差")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_get_hand_gripper_clockwise(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_joint: {case['joint']}")

    with allure.step("发送请求，获取指定夹爪顺时针方向"):
        response = device.ml.get_hand_gripper_clockwise(case["joint"])
...
def test_get_hand_gripper_clockwise_exception(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_joint: {case['joint']}")

    with allure.step(f"尝试传入非法 joint 值，期望抛出 MercuryDataException,关节为{case['joint']}"):
        with pytest.raises(MercuryDataException):
            device.ml.get_hand_gripper_clockwise(case["joint"])
