...
# 从 Excel 提取用例
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "set_gripper_protect_current")
...
def device():
    ...
    dev.mc.set_gripper_protect_current(dev.hts_gripper_protect_current)
    dev.mc.close()
    logger.info("环境清理完成，接口测试结束")


@allure.feature("set_gripper_protect_current 接口测试")
@allure.story("正常 set_gripper_protect_current 场景")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_gripper_protect_current_normal(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_gripper_protect_current(case['parameter'])

    with allure.step('调用 get_gripper_protect_current 接口'):
        get_res = device.mc.get_gripper_protect_current()
    ...
    with allure.step('断言 get_gripper_protect_current 返回结果'):
...
def test_set_gripper_protect_current_exception(device, case):
    title = case["title"]
    ...
    with allure.step("断言抛出 MyCobot280DataException"):
        with pytest.raises(MyCobot280DataException):
            device.mc.set_gripper_protect_current(param)
