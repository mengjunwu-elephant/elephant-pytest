...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "jog_angle")
...
@allure.story("插补模式设置jog_angle")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_jog_angle0(device, case):
    title = case["title"]
    ...
    with allure.step('设置2关节角度时，调整3关节姿态，设置4关节角度时，调整5关节姿态,防止碰撞'):
        if case["joint"] == 2 and case["position"] == 0:
            device.mc.send_angle(3, 90, device.speed)
            device.wait()
        elif case["joint"] == 2 and case["position"] == 1:
            device.mc.send_angle(3, -90, device.speed)
            device.wait()
        elif case["joint"] == 4:
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.jog_angle(case["joint"],case["position"],case["speed"])
        device.wait()
    ...
    with allure.step("断言 get_angle 返回值"):
        ...
        assert_almost_equal(get_res, case["target_angle"], 2,'插补模式设置jog_angle'), f"用例【{title}】断言失败，期望 {case['target_angle']},实际 {get_res}"
...
def test_jog_angle1(device, case):
    ...
    with allure.step("断言 get_angle 返回值"):
        allure.attach(str(case["target_angle"]), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, case["target_angle"], 2,'刷新模式设置jog_angle'), f"用例【{title}】断言失败，期望 {case['target_angle']},实际 {get_res}"
...
def test_jog_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 MyCobot280DataException,关节为{case['joint']},方向为{case['position']}, 速度为{case['speed']}"):
        with pytest.raises(MyCobot280DataException):
            device.mc.jog_angle(case["joint"],case["position"], case["speed"])
