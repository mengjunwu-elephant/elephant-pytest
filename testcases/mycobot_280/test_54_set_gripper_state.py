...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "set_gripper_state")
...
@allure.feature("单个电机掉电")
@allure.story("设置单个电机掉电")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_gripper_state(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_gripper_state 接口"):
        response = device.mc.set_gripper_state(case['flag'],case['speed'],case['type'],case['is_torque'])
        logger.debug(f"接口返回：{response}")

    with allure.step(f'手动查看{case["joint"]}关节舵机是否释放'):
        input(f'请手动拖动{case["joint"]}关节舵机，查看是否放松,按回车键继续')

    with allure.step("锁紧对应关节"):
...
def test_set_gripper_state_exception(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
