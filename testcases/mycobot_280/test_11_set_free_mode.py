...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "set_free_mode")
...
@allure.feature("设置自由模式")
@allure.story("正确设置自由模式")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_free_mode(device, case):
    title = case["title"]
    expected = case["expect_data"]
    mode = '自由' if case["parameters"] == 1 else '非自由'

    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')
    logger.debug(f'parameters:{case["parameters"]}')

    with allure.step("调用 set_free_mode 接口"):
        response = device.mc.set_free_mode(case['parameters'])
        logger.debug(f"接口返回：{response}")

    with allure.step('查看当前是否为自由模式'):
        input(print(f'当前模式为{mode},请按住末端按钮查看是否能够手动拖动机械臂'))
...
def test_set_free_mode_exception(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
