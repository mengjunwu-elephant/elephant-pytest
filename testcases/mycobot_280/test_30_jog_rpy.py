...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "jog_rpy")
...
@allure.feature("RPY旋转")
@allure.story("插补模式设置jog_rpy")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_jog_rpy0(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.jog_rpy(case["axis"],case["position"],case["speed"])
        device.wait()
...
def test_jog_rpy_exception(device, case):
    ...
    with allure.step(f"断言抛出 MyCobot280DataException,笛卡尔积坐标系为{case['axis']},方向为{case['position']}, 速度为{case['speed']}"):
