...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "get_angles_plan")
...
@allure.story("插补模式角度规划值")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_angles_plan0(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        get_res = device.mc.get_angles_plan()
        logger.debug(f"接口返回：{get_res}")

    with allure.step(f'调用 send_angles 接口'):
        device.mc.send_angles(get_res, device.speed)
    ...
    with allure.step("断言接口返回结果"):
        allure.attach(str(device.coords_init_angles), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, device.coords_init_angles,0.1,'获取关节角度规划值') , f"用例【{title}】断言失败，期望 {device.coords_init_angles},实际 {get_res}"
...
def test_get_angles_plan1(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step("断言接口返回结果"):
        allure.attach(str(device.coords_init_angles), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, device.coords_init_angles, 0.1,
                            '获取关节角度规划值'), f"用例【{title}】断言失败，期望 {device.coords_init_angles},实际 {get_res}"
