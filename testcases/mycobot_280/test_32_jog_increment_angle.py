...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "jog_increment_angle")
...
def test_jog_increment_angle0(device, case):
    ...
    with allure.step("断言 get_angle 返回值"):
        ...
        assert_almost_equal(get_res, case["increment"], 2,'插补模式设置jog_increment_angle'), f"用例【{title}】断言失败，期望 {case['increment']},实际 {get_res}"
...
def test_jog_increment_angle1(device, case):
    ...
    with allure.step("断言 get_angle 返回值"):
        allure.attach(str(case["increment"]), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, case["increment"], 2,'刷新模式设置jog_increment_angle'), f"用例【{title}】断言失败，期望 {case['increment']},实际 {get_res}"
