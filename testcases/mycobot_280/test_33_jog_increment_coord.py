...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "jog_increment_coord")
...
def test_jog_increment_coord0(device, case):
    ...
    with allure.step("断言 get_coords 返回值"):
        ...
        assert_almost_equal(target_get_res, init_get_res + case["increment"], 5,
                            '插补模式设置jog_increment_coord'), f"用例【{title}】断言失败，期望 {init_get_res + case['increment']},实际 {target_get_res}"
...
def test_jog_increment_coord_exception(device, case):
    ...
    with allure.step(
            f"断言抛出 MyCobot280DataException,关节为{case['axis']},增量为{case['increment']}, 速度为{case['speed']}"):
