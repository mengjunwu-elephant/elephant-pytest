...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "release_servo")
...
def test_release_servo(device, case):
    title = case["title"]
    ...
    with allure.step("调用 release_servo 接口"):
        response = device.mc.release_servo(case['joint'])
...
def test_release_servo_exception(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
