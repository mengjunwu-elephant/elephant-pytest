...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "jog_coord")
...
@allure.story("插补模式设置jog_coord")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_jog_coord0(device, case):
    title = case["title"]
    ...
    with allure.step('将机械臂运动到坐标初始姿态'):
        device.mc.send_angles(device.coords_init_angles, device.speed)
        device.wait()

    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.jog_coord(case["axis"],case["position"],case["speed"])
        device.wait()
...
def test_jog_coord_exception(device, case):
    ...
    with allure.step(f"断言抛出 MyCobot280DataException,笛卡尔坐标系为{case['axis']},方向为{case['position']}, 速度为{case['speed']}"):
        with pytest.raises(MyCobot280DataException) as exc:
            device.mc.jog_coord(case["axis"],case["position"], case["speed"])
