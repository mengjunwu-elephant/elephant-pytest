...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "set_joint_max")
...
def test_set_joint_max_angle1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.set_joint_max(case["joint"],case["angle"])
    ...
    with allure.step("断言get_angle接口返回结果"):
        ...
        assert_almost_equal(angle, case['angle'], 2,'设置关节最大角度'), f"用例【{title}】断言失败，期望 {case['angle']},实际 {angle}"
...
def test_set_joint_max_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 MyCobot280DataException,关节为{case['joint']},角度为{case['angle']}"):
        with pytest.raises(MyCobot280DataException):
            device.mc.set_joint_max(case['joint'],case['angle'])
