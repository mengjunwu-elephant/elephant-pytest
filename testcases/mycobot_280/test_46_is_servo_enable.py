...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot280Base.TEST_DATA_FILE, "is_servo_enable")
...
@allure.feature("查看单关节连接状态")
@allure.story("获取单关节连接状态")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_is_servo_enable(device, case):
    title = case["title"]
    ...
    with allure.step("调用 is_servo_enable 接口"):
        response = device.mc.is_servo_enable(case['joint'])
...
def test_is_servo_enable_exception(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
