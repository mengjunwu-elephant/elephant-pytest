...
cases = get_test_data_from_excel(UltraArmP1Base.ATTACHMENTS_TEST_DATA_FILE, "quick_off_laser")
...
def test_quick_off_laser(device, case):
    ...
    with allure.step(f"调用 {case['api']}（pymycobot: set_pwm_laser_mode）"):
        response = device.mc.set_pwm_laser_mode(int(case["state"]))
...
@allure.feature("PWM激光")
@allure.story("quick_off_laser 参数越界")
@pytest.mark.parametrize(
    "case",
    [c for c in cases if c.get("test_type") == "exception"],
    ids=lambda c: c["title"],
)
def test_quick_off_laser_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException, state: {case['state']}"):
        with pytest.raises(ultraArmP1DataException) as exc:
            device.mc.set_pwm_laser_mode(int(case["state"]))
