...
def test_set_gripper_parameter_normal(device, case):
    ...
    with allure.step("调用获取接口"):
        get_res = device.mc.get_gripper_parameter(case["parameter"])
    ...
    with allure.step("断言获取值与设置值一致"):
        ...
        assert get_res == case["value"],f"期望：{case['value']}，实际：{get_res}"
...
def test_set_gripper_parameter_exception(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_parameter: {case['parameter']}")
    logger.debug(f"test_value: {case['value']}")
    ...
    with allure.step(f"断言设置接口抛出 ultraArmP1DataException, value: {case['value']}"):
        with pytest.raises(ultraArmP1DataException):
            device.mc.set_gripper_parameter(case["parameter"], case["value"])
