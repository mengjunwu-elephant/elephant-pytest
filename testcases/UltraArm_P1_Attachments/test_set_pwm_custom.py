...
cases = get_test_data_from_excel(UltraArmP1Base.ATTACHMENTS_TEST_DATA_FILE, "set_pwm_custom")
...
@allure.story("set_pwm_custom 设置自定义档位")
...
def test_set_pwm_custom(device, case):
    ...
    with allure.step("先打开自定义PWM模式"):
        device.mc.set_pwm_custom_mode(1)

    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.set_pwm_custom(int(case["p_value"]))
...
@allure.feature("PWM激光")
@allure.story("set_pwm_custom 参数越界")
@pytest.mark.parametrize(
    "case",
    [c for c in cases if c.get("test_type") == "exception"],
    ids=lambda c: c["title"],
)
def test_set_pwm_custom_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException, p_value: {case['p_value']}"):
        with pytest.raises(ultraArmP1DataException) as exc:
            device.mc.set_pwm_custom(int(case["p_value"]))
