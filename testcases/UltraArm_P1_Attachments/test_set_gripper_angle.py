...
import pytest
import allure
from pymycobot.error import MercuryDataException, ultraArmP1DataException
...
# 加载测试数据
cases = get_test_data_from_excel(UltraArmP1Base.ATTACHMENTS_TEST_DATA_FILE, "set_gripper_angle")
...
def test_set_gripper_angle_exception(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_value: {case['value']}")
