...
cases = get_test_data_from_excel(UltraArmP1Base.ATTACHMENTS_TEST_DATA_FILE, "set_pwm_laser")
...
@allure.story("set_pwm_laser 设置激光档位")
...
def test_set_pwm_laser(device, case):
    ...
    with allure.step("先打开激光PWM模式"):
        device.mc.set_pwm_laser_mode(1)

    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.set_pwm_laser(int(case["p_value"]))
...
@allure.feature("PWM激光")
@allure.story("set_pwm_laser 参数越界")
@pytest.mark.parametrize(
    "case",
    [c for c in cases if c.get("test_type") == "exception"],
    ids=lambda c: c["title"],
)
def test_set_pwm_laser_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException, p_value: {case['p_value']}"):
        with pytest.raises(ultraArmP1DataException) as exc:
            device.mc.set_pwm_laser(int(case["p_value"]))
