...
def test_get_gripper_run_status_normal(device, case):
    title = case["title"]
    expected = case["expect_data"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"接口: {case['api']}，参数: {case['parameter']}")

    with allure.step("获取夹爪运动状态"):
        response = device.mc.get_gripper_run_status()
...
def test_get_gripper_run_status_normal1(device, case):
    title = case["title"]
    expected = case["expect_data"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"接口: {case['api']}，参数: {case['parameter']}")
    sleep(5)
    with allure.step("使夹爪运动"):
        device.mc.set_gripper_angle(100,1)
        sleep(1)
