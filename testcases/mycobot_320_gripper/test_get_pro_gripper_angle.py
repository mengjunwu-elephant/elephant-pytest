import time

import pytest
import allure
from common1.test_data_handler import get_test_data_from_excel
from common1 import logger, assert_almost_equal
from settings import Mycobot320Base

cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "get_pro_gripper_angle")

normal_cases = [case for case in cases if case.get("test_type") == "normal"]

@pytest.fixture(scope="module")
def device():
    dev = Mycobot320Base()
    logger.info("初始化完成，接口测试开始")
    yield dev
    dev.m.set_pro_gripper_angle(0)
    dev.m.close()
    logger.info("环境清理完成，接口测试结束")

@allure.feature("获取Pro夹爪角度")
@allure.story("正常用例")
@pytest.mark.parametrize("case", normal_cases, ids=[case["title"] for case in normal_cases])
def test_get_pro_gripper_angle(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")

    with allure.step("调用接口设置夹爪角度"):
        device.m.set_pro_gripper_angle(case['angles_set'])
        time.sleep(2)

    with allure.step("调用接口获取夹爪角度"):
        response = device.m.get_pro_gripper_angle()
        logger.debug(f"接口返回值: {response}")
        allure.attach(str(response), "接口返回值", allure.attachment_type.TEXT)

    with allure.step("断言返回类型为 int"):
        assert isinstance(response, int), f"返回类型错误，实际为 {type(response)}"

    with allure.step("断言返回值与期望值一致"):
        allure.attach(str(case['expect_data']), "期望值", allure.attachment_type.TEXT)
        allure.attach(str(response), "实际值", allure.attachment_type.TEXT)
        assert_almost_equal(response, case['expect_data'], tol=2) #tol代表允许的误差值

    logger.info(f"✅ 用例【{case['title']}】测试成功")
    logger.info(f"》》》用例【{case['title']}】测试完成《《《")
