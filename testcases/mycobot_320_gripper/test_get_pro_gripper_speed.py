...
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "get_pro_gripper_speed")
...
@allure.feature("获取Pro夹爪速度")
@allure.story("全部用例")
@pytest.mark.parametrize("case", normal_cases, ids=[case["title"] for case in normal_cases])
def test_get_pro_gripper_speed(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")

    with allure.step("调用接口设置夹爪速度"):
        device.m.set_pro_gripper_speed(case['speed_set'])

    with allure.step("调用接口获取夹爪速度"):
        response = device.m.get_pro_gripper_speed()
