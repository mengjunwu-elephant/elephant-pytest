...
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "get_pro_gripper_status")
...
def device():
    ...
    yield dev
    dev.m.set_pro_gripper_angle(0)  # 回到零位
    dev.m.close()
...
def test_get_pro_gripper_status_0(device, case):
    ...
    with allure.step("获取夹爪状态"):
        response = device.m.get_pro_gripper_status()
...
def test_get_pro_gripper_status_2(device, case):
    ...
    with allure.step("设置夹爪角度为 0"):
        device.m.set_pro_gripper_angle(0)
        sleep(3)
