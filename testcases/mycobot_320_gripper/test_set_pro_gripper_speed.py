...
# 从 Excel 中提取测试数据
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "set_pro_gripper_speed")
...
def device():
    ...
    yield dev
    dev.m.set_pro_gripper_speed(100)
    dev.m.m.close()
...
def test_set_pro_gripper_speed_valid(device, case):
    ...
    with allure.step("调用 set_pro_gripper_speed 接口"):
        response = device.m.set_pro_gripper_speed(case["parameter"])
...
def test_set_pro_gripper_speed_exception(device, case):
    ...
    with allure.step(f"调用接口并断言抛出 MercuryDateException 异常,速度为{case['parameter']}"):
        with pytest.raises(MercuryDataException):
            device.m.set_pro_gripper_speed(case["parameter"])
