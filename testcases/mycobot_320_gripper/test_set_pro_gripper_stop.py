...
# 从Excel中提取测试数据
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "set_pro_gripper_stop")
...
def device():
    ...
    yield dev
    dev.m.set_pro_gripper_abs_angle(0)
    dev.m.m.close()
...
def test_set_pro_gripper_stop(device, case):
    ...
    with allure.step("设置夹爪绝对角度（模拟夹爪在动作中）"):
        device.m.set_pro_gripper_abs_angle(100)

    with allure.step("调用 set_pro_gripper_stop 接口"):
        response = device.m.set_pro_gripper_stop()
