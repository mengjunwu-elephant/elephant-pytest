...
# 提取测试数据
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "set_pro_gripper_torque")
...
def device():
    ...
    yield dev
    dev.m.set_pro_gripper_torque(100)
    dev.m.m.close()
...
def test_set_pro_gripper_torque_exception(device, case):
    ...
    with allure.step(f"断言设置异常值时抛出 MercuryDataException，扭矩为{case['parameter']}"):
        with pytest.raises(MercuryDataException):
            device.m.set_pro_gripper_torque(case["parameter"])
