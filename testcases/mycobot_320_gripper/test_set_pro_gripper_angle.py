...
# 加载测试数据
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "set_pro_gripper_angle")
...
def test_set_pro_gripper_angle_normal(device, case):
    ...
    with allure.step("调用设置接口"):
        set_res = device.m.set_pro_gripper_angle(case["value"])
        allure.attach(str(set_res), "设置接口返回值", allure.attachment_type.TEXT)

    with allure.step("调用获取接口"):
        get_res = device.m.get_pro_gripper_angle()
...
def test_set_pro_gripper_angle_exception(device, case):
    ...
    with allure.step(f"断言设置接口抛出 MercuryDataException, value: {case['value']}"):
        with pytest.raises(MercuryDataException):
            device.m.set_pro_gripper_angle(case["value"])
