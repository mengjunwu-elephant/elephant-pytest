...
# 从 Excel 中提取测试数据
cases = get_test_data_from_excel(Mycobot320Base.GRIPPER_TEST_DATA_FILE, "set_pro_gripper_m.close")


@pytest.fixture(scope="module")
def device():
    dev = Mycobot320Base()
    dev.m.set_pro_gripper_open()
...
@allure.feature("设置Pro夹爪关闭")
@allure.story("夹爪闭合功能验证")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_set_pro_gripper_m.close(device, case):
    logger.info(f"》》》用例【{case['title']}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
    logger.debug(f"test_parameters: {case.get('parameter', '')}")

    with allure.step("调用 set_pro_gripper_m.close 接口"):
        response = device.m.set_pro_gripper_m.close()
