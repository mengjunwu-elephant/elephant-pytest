...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_tool_reference")
...
@allure.feature("获取工具坐标系")
@allure.story("获取工具坐标系")
...
def test_get_tool_reference1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_tool_reference()
