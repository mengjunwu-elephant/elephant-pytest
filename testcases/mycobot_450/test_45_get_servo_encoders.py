...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_servo_encoders")
...
@allure.feature("读取全关节编码值")
@allure.story("正确读取全关节编码值")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_servo_encoders1(device, case):
    title = case["title"]
