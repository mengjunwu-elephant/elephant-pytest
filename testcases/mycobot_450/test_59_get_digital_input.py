...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_digital_input")
...
def test_get_digital_input_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,引脚为{case['pin_no']}"):
        with pytest.raises(MyCobotPro450DataException) as exc:
            device.mc.get_digital_input(case['pin_no'])
