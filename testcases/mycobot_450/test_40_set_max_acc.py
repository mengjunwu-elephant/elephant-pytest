...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_max_acc")
...
@allure.story("正确设置最大速度")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_max_acc1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.set_max_acc(case["mode"],case["parameter"])
...
def test_set_max_acc_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,模式为{case['mode']},参数为{case['parameter']}"):
