...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_servo_currents")
...
@allure.story("获取静止状态下关节电流")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_servo_currents1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_servo_currents()
    ...
    with allure.step("断言接口返回结果"):
        ...
        assert_almost_equal(response, expected, 5, "获取关节静止状态下电流"),f"用例【{title}】断言失败，期望 {expected},实际 {response}"

    logger.info(f'✅ 用例【{title}】测试通过')
    logger.info(f'》》》》》用例【{case["title"]}】测试完成《《《《《')

@allure.feature("获取关节电流")
@allure.story("获取运动状态下关节电流")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal1"], ids=lambda c: c["title"])
def test_get_servo_currents2(device, case):
    title = case["title"]
    expected = eval(case["expect_data"])
