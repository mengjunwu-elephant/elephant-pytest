...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "jog_rpy")
...
def test_jog_rpy_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,笛卡尔积坐标系为{case['axis']},方向为{case['position']}, 速度为{case['speed']}"):
