...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_torque_comp")
...
def device():
    ...
    yield dev
    dev.default_torque_comp()
    dev.mc.close()
    logger.info("环境清理完成，接口测试结束")

@allure.feature("设置力矩补偿参数")
@allure.story("正确设置力矩补偿参数")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_torque_comp1(device, case):
    title = case["title"]
    expected = case["expect_data"]
    joint = case["joint"]
    torque_comp = case["torque_comp"]
    comp_value = case["comp_value"]

    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')
    logger.debug(f'joint:{case["joint"]}')
    logger.debug(f'torque_comp:{case["torque_comp"]}')
    logger.debug(f'comp_value:{case["comp_value"]}')

    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_torque_comp(joint, comp_value, torque_comp)
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 get_torque_comp 接口'):
    ...
    with allure.step('断言是否设置成功'):
        allure.attach(str(torque_comp), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res[joint-1]), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert get_res[joint-1] == torque_comp, f"用例【{title}】断言失败，期望 {torque_comp},实际 {get_res[joint-1]}"

    logger.info(f'✅ 用例【{title}】测试通过')
    logger.info(f'》》》》》用例【{case["title"]}】测试完成《《《《《')

@allure.feature("设置力矩参数")
@allure.story("超限参数验证")
...
def test_set_torque_comp_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,关节为{joint}，力矩参数{torque_comp}，阻抗开关{comp_value}"):
