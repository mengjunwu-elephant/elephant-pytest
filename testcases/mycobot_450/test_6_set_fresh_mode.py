...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_fresh_mode")
...
def test_set_fresh_mode1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_fresh_mode(case["mode"])
        logger.debug(f"接口返回：{set_res}")

    with allure.step(f'调用 get_fresh_mode 接口'):
        get_res = device.mc.get_fresh_mode()
...
def test_set_fresh_mode_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,模式为{case['mode']}"):
        with pytest.raises(MyCobotPro450DataException):
            device.mc.set_fresh_mode(case['mode'])
