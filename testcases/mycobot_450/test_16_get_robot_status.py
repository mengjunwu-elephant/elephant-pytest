...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_robot_status")


@allure.feature("查看机器人状态")
@allure.story("查看机器人状态（无报错）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_robot_status(device, case):
    title = case["title"]
