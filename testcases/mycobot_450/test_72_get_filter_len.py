...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_filter_len")
...
def test_get_filter_len1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_filter_len(case['rank'])
...
def test_get_filter_len_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,参数为{case['rank']}"):
        with pytest.raises(MyCobotPro450DataException):
            device.mc.get_filter_len(case['rank'])
