...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "get_motors_run_err")
...
@allure.feature("读取运动中电机报错")
@allure.story("正确读取运动中电机报错（无报错）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_motors_run_err1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_motors_run_err()
