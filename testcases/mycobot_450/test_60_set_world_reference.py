...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_world_reference")
...
def test_set_world_reference1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_world_reference(eval(case["coords"]))
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 set_reference_frame 接口'):
        device.mc.set_reference_frame(1)

    with allure.step('调用 get_world_reference 接口'):
        get_res = device.mc.get_world_reference()
