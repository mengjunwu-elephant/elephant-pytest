...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_joint_min_angle")
...
@allure.story("正确设置关节最小角度")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_joint_min_angle1(device, case):
    title = case["title"]
...
def test_set_joint_min_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,关节为{case['joint']},角度为{case['angle']}"):
