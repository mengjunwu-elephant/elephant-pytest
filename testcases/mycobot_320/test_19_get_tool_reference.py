...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_tool_reference")
...
def test_get_tool_reference1(device, case):
    ...
    device.m.set_tool_reference([1,1,1,1,1,1])
    time.sleep(0.5)

    with allure.step("调用 get_tool_reference 接口"):
        response = device.m.get_tool_reference()
        logger.debug(f"接口返回：{response}")

    device.m.set_tool_reference(device.zero_angles)
    time.sleep(0.5)
...
def test_get_tool_reference2(device, case):
    title = case["title"]
    expected = case["expect_data"]
