...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_servo_voltages")
...
def test_get_servo_voltages1(device, case):
    ...
    device.m.send_angles([45, 45, -90, 45, 90, 90], device.speed)
    time.sleep(0.1)
    while True:
        if device.m.is_moving() == 1:
            break
    time.sleep(0.5)

    with allure.step("调用 get_servo_voltages 接口"):
        response = device.m.get_servo_voltages()
    ...
    with allure.step("断言接口返回结果"):
        ...
        device.range_comparison(eval(expected), response, title)
...
def test_get_servo_voltages2(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    time.sleep(2)
    device.m.send_angles(device.zero_angles, 100)
    time.sleep(0.1)
    while True:
        if device.m.is_moving() == 1:
            break
