...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_servo_data")
...
def test_get_servo_data1(device, case):
    ...
    logger.debug(f'data_id:{case["data_id"]}')
    logger.debug(f'mode:{case["mode"]}')

    device.m.set_servo_data(joint, data_id, 30, mode)
    time.sleep(0.5)

    with allure.step("调用 get_servo_data 接口"):
        response = device.m.get_servo_data(joint, data_id, mode)
        logger.debug(f"接口返回：{response}")

    device.m.set_servo_data(joint, data_id, 32, mode)
    time.sleep(0.5)
...
def test_get_servo_data2(device, case):
    title = case["title"]
    expected = case["expect_data"]
    joint = case["joint"]
    data_id = case["data_id"]
    ...
    if data_id == 21:
        with pytest.raises(MyCobot320DataException, match=".*"):
            device.m.get_servo_data(joint, data_id, mode)
    else:
...
def test_get_servo_data3(device, case):
    title = case["title"]
    expected = case["expect_data"]
    joint = case["joint"]
    data_id = case["data_id"]
