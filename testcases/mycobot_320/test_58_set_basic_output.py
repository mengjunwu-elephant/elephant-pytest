...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_basic_output")
...
def test_set_basic_output1(device, case):
    ...
    with allure.step("调用 set_basic_output 接口"):
        set_res = device.m.set_basic_output(pin_no, pin_signal)
        time.sleep(0.1)
        get_res = device.m.get_basic_input(pin_no)
    ...
    with allure.step("断言 set_basic_output 返回结果"):
    ...
    with allure.step("断言 get_basic_input 返回结果"):
...
def test_set_basic_output2(device, case):
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_basic_output(pin_no, pin_signal)
