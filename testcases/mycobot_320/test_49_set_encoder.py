...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_encoder")
...
def test_set_encoder1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 set_encoder 接口, joint = {case['joint']}, encoder = {case['encoder']}, speed = {case['speed']}"):
        set_res = device.m.set_encoder(joint, encoder, speed)
        time.sleep(4)
        get_res = device.m.get_encoder(joint)
    ...
    with allure.step("断言 set_encoder 返回值"):
    ...
    with allure.step("断言 get_encoder 返回值"):
        allure.attach(str(encoder), name="get接口期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="get接口实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, encoder, tol=22) #tol代表允许的误差值
...
def test_set_encoder2(device, case):
    title = case["title"]
    joint = case["joint"]
    encoder = case["encoder"]
    speed = case["speed"]
