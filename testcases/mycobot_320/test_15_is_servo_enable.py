...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "is_servo_enable")
...
def test_is_servo_enable1(device, case):
    ...
    with allure.step("调用 is_servo_enable 接口"):
        response = device.m.is_servo_enable(joint)
...
def test_is_servo_enable2(device, case):
    title = case["title"]
    joint = case["joint"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.is_servo_enable(joint)
...
def test_is_servo_enable3(device, case):
    title = case["title"]
    expected = case["expect_data"]
    joint = case["joint"]
