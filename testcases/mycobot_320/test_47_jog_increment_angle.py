...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "jog_increment_angle")
...
def test_jog_increment_angle1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 jog_increment_angle 接口, joint = {case['joint']}, increment = {case['increment']}, speed = {case['speed']}"):
        get_res1 = device.m.get_angles()
        set_res = device.m.jog_increment_angle(joint, increment, speed)
        device.wait()
        get_res2 = device.m.get_angles()
    ...
    with allure.step("断言 jog_increment_angle 设置返回值"):
    ...
    with allure.step("断言 get_angles 获取返回值"):
        ...
        assert_almost_equal(get_res, expected_2, tol=2)  # tol代表允许的误差值
...
def test_jog_increment_angle2(device, case):
    title = case["title"]
    joint = case["joint"]
    increment = case["increment"]
    speed = case["speed"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.jog_increment_angle(joint, increment, speed)
