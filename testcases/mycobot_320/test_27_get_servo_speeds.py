...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_servo_speeds")
...
def test_get_servo_speeds1(device, case):
    ...
    with allure.step("调用 get_servo_speeds 接口"):
        response = device.m.get_servo_speeds()
...
def test_get_servo_speeds2(device, case):
    title = case["title"]
    expected = case["expect_data"]
