...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_servo_last_pdi")
...
def test_get_servo_last_pdi1(device, case):
    ...
    with allure.step("调用 get_servo_last_pdi 接口"):
        response = device.m.get_servo_last_pdi(joint)
...
def test_get_servo_last_pdi2(device, case):
    title = case["title"]
    joint = case["joint"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.get_servo_last_pdi(joint)
