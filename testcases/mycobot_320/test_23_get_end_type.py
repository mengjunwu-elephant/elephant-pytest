...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_end_type")
...
def test_get_end_type1(device, case):
    ...
    with allure.step("调用 get_end_type 接口"):
        response = device.m.get_end_type()
...
def test_get_end_type2(device, case):
    title = case["title"]
    expected = case["expect_data"]
