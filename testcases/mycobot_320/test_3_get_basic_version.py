...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_basic_version")
...
def test_get_basic_version1(device, case):
    ...
    with allure.step("调用 get_basic_version 接口"):
        response = device.m.get_basic_version()
...
def test_get_basic_version2(device, case):
    title = case["title"]
    expected = case["expect_data"]
