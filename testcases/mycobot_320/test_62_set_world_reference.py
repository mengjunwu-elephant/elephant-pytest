...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_world_reference")
...
def test_set_world_reference1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_world_reference 接口"):
        ...
        set_res = device.m.set_world_reference(eval(coords))
        get_res_w = device.m.get_world_reference()
        time.sleep(0.1)
        get_res2 = device.m.get_coords()
        get_res_c = get_res2[bit-1] - get_res1[bit-1]
        device.m.set_world_reference([0, 0, 0, 0, 0, 0])
        logger.debug(f"set_res返回:{set_res},get_res_t返回:{get_res_w},get_res_c返回:{get_res_c}")
    ...
    with allure.step("断言 set_world_reference 返回结果"):
    ...
    with allure.step("断言 get_world_reference 返回结果"):
        allure.attach(str(coords), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res_w), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert get_res_w == eval(coords), f"用例【{title}】断言失败，期望 {eval(coords)}，实际 {get_res_w}"
...
def test_set_world_reference2(device, case):
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_world_reference(coords)
