...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_system_version")
...
def test_get_system_version1(device, case):
    ...
    with allure.step("调用 get_system_version 接口"):
        response = device.m.get_system_version()
...
def test_get_system_version2(device, case):
    title = case["title"]
    expected = case["expect_data"]
