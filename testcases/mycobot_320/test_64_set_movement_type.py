...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_movement_type")
...
def test_set_movement_type1(device, case):
    title = case["title"]
    expected_1 = case["expect_data_1"]
    move_type = case["move_type"]

    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')
    logger.debug(f'test_move_type:{case["move_type"]}')

    with allure.step("调用 set_movement_type 接口"):
        set_res = device.m.set_movement_type(move_type)
        time.sleep(0.1)
        get_res = device.m.get_movement_type()
        logger.debug(f"set_res返回:{set_res},get_res:{get_res}")

    with allure.step("设置末端坐标系类型为法兰"):
        device.m.set_movement_type(0)

    with allure.step("断言返回值类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误,应为{type(expected_1)},实际为 {type(set_res)}"

    with allure.step("断言 set_movement_type 返回结果"):
    ...
    with allure.step("断言 get_movement_type 返回结果"):
        allure.attach(str(move_type), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert get_res == move_type, f"用例【{title}】断言失败，期望 {move_type}，实际 {get_res}"
...
def test_set_movement_type2(device, case):
    title = case["title"]
    move_type = case["move_type"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_movement_type(move_type)
