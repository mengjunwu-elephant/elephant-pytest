...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_movement_type")
...
def test_get_movement_type1(device, case):
    ...
    with allure.step("调用 get_movement_type 接口"):
        response = device.m.get_movement_type()
        logger.debug(f"接口返回：{response}")

    device.m.set_movement_type(1)
    time.sleep(0.5)
...
def test_get_movement_type2(device, case):
    title = case["title"]
    expected = case["expect_data"]
