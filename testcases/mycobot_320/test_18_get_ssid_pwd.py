...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_ssid_pwd")
...
def test_get_ssid_pwd1(device, case):
    ...
    device.m.set_ssid_pwd('123','123')
    time.sleep(0.5)

    with allure.step("调用 get_ssid_pwd 接口"):
        response = device.m.get_ssid_pwd()
        logger.debug(f"接口返回：{response}")

    device.m.set_ssid_pwd('MyCobotWiFi2.4G','mycobot123')
    time.sleep(0.5)
...
def test_get_ssid_pwd2(device, case):
    title = case["title"]
    expected = case["expect_data"]
