...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_ssid_pwd")
...
def test_set_ssid_pwd1(device, case):
    ...
    with allure.step("调用 set_ssid_pwd 接口"):
        set_res = device.m.set_ssid_pwd(account, password)
        time.sleep(0.1)
        get_res = device.m.get_ssid_pwd()
    ...
    with allure.step("断言 set_ssid_pwd 返回结果"):
    ...
    with allure.step("断言 get_ssid_pwd 返回结果"):
        ...
        assert get_res == eval(expected_2), f"用例【{title}】断言失败，期望 {expected_2}，实际 {get_res}"

    logger.info(f'✅ 用例【{title}】测试通过')
    logger.info(f'》》》》》用例【{case["title"]}】测试完成《《《《《')

@allure.feature("设置int类型WiFi账号密码")
@allure.story("异常用例")
@pytest.mark.parametrize("case", exception_cases, ids=[case["title"] for case in exception_cases])
def test_set_ssid_pwd2(device, case):
    title = case["title"]
    account = case["account"]
    password = case["password"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_ssid_pwd(account, password)
