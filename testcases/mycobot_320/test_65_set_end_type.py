...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_end_type")
...
def test_set_end_type1(device, case):
    title = case["title"]
    ...
    with allure.step("断言 get_end_type 返回结果"):
        allure.attach(str(end), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res_e), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert get_res_e == end, f"用例【{title}】断言失败，期望 {end}，实际 {get_res_e}"
