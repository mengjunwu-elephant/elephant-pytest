...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "send_coord")
...
def test_send_coord1(device, case):
    ...
    with allure.step(f"调用 send_coord 接口, joint = {case['joint']}, coord = {case['coord']}, speed = {case['speed']}"):
        set_res = device.m.send_coord(joint, coord, speed)
        time.sleep(0.1)
        while True:
            if device.m.is_moving() == 0:
                break
    ...
    with allure.step("断言获取返回值"):
        allure.attach(str(coord), name="get接口期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res[joint-1]), name="get接口实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res[joint-1], coord, tol=5) #tol代表允许的误差值
...
def test_send_coord2(device, case):
    title = case["title"]
    joint = case["joint"]
    coord = case["coord"]
    speed = case["speed"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.send_coord(joint, coord, speed)
