...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "resume")
...
def test_resume1(device, case):
    ...
    with allure.step(f"调用 pause 接口"):
        device.m.pause()
        get_res1 = device.m.is_paused()
        set_res = device.m.resume()
        get_res2 = device.m.is_paused()
    ...
    with allure.step("断言恢复运动后is_paused返回值"):
