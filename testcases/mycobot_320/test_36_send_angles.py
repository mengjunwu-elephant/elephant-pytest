...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "send_angles")
...
def test_send_angles1(device, case):
    ...
    with allure.step(f"调用 send_angles 接口, angles = {case['angles']}, speed = {case['speed']}"):
        set_res = device.m.send_angles(eval(angles), speed)
        time.sleep(0.1)
        while True:
    ...
    with allure.step("断言获取返回值"):
        allure.attach(str(angles), name="get接口期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="get接口实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, eval(angles), tol=2) #tol代表允许的误差值
...
def test_send_angles2(device, case):
    title = case["title"]
    angles = case["angles"]
    speed = case["speed"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.send_angles(eval(angles), speed)
...
def test_send_angles3(device, case):
    title = case["title"]
    expected = case["expect_data"]
    angles = case["angles"]
    speed = case["speed"]
