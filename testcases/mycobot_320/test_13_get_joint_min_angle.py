...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_joint_min_angle")
...
def test_get_joint_min_angle1(device, case):
    ...
    with allure.step("调用 get_joint_min_angle 接口"):
        response = device.m.get_joint_min_angle(joint)
...
def test_get_joint_min_angle2(device, case):
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.get_joint_min_angle(joint)
