...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_tool_reference")
...
def test_set_tool_reference1(device, case):
    ...
    logger.debug(f'test_coords:{case["coords"]}')
    logger.debug(f'test_angles:{case["angles"]}')

    with allure.step("使机械臂运动到指定位置"):
        device.m.send_angles(eval(angles), device.speed)
        device.wait()

    with allure.step("设置末端坐标系类型为工具"):
        device.m.set_end_type(1)

    with allure.step("调用 set_tool_reference 接口"):
        get_res1 = device.m.get_coords()
        set_res = device.m.set_tool_reference(eval(coords))
        get_res_t = device.m.get_tool_reference()
        time.sleep(0.1)
        get_res2 = device.m.get_coords()
        get_res_c = get_res2[bit-1] - get_res1[bit-1]
        device.m.set_tool_reference([0, 0, 0, 0, 0, 0])
        logger.debug(f"set_res返回:{set_res},get_res_t返回:{get_res_t},get_res_c返回:{get_res_c}")

    with allure.step("设置末端坐标系类型为法兰"):
        device.m.set_end_type(0)

    with allure.step("断言返回值类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误,应为{type(expected_1)},实际为 {type(set_res)}"

    with allure.step("断言 set_tool_reference 返回结果"):
    ...
    with allure.step("断言 get_tool_reference 返回结果"):
        allure.attach(str(coords), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res_t), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert get_res_t == eval(coords), f"用例【{title}】断言失败，期望 {eval(coords)}，实际 {get_res_t}"
...
def test_set_tool_reference2(device, case):
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_tool_reference(coords)
