...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_joint_min")
...
def test_set_joint_min1(device, case):
    ...
    with allure.step("调用 set_joint_min 接口"):
    ...
    with allure.step("断言 set_joint_min 返回结果"):
    ...
    with allure.step("断言 get_joint_min_angle 返回结果"):
...
def test_set_joint_min2(device, case):
    title = case["title"]
    expected_1 = case["expect_data_1"]
    expected_2 = case["expect_data_2"]
    angle = case["angle"]
...
def test_set_joint_min3(device, case):
    title = case["title"]
    joint = case["joint"]
    angle = case["angle"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_joint_min(joint, angle)
