...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_server_port")
...
def test_set_server_port1(device, case):
    ...
    with allure.step("调用 set_server_port 接口"):
        set_res = device.m.set_server_port(port)
        logger.debug(f"set_res返回:{set_res}")
        get_res = input(f"设置端口为{port},机械臂使用WiFi连接,查看端口是否设置成功,成功输入1,失败输入2:\n")

    with allure.step("断言返回值类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误,应为{type(expected_1)},实际为 {type(set_res)}"

    with allure.step("断言 set_server_port 返回结果"):
    ...
    with allure.step("断言 get_ssid_pwd 返回结果"):
        ...
        assert int(get_res) == expected_2, f"用例【{title}】断言失败，期望 {expected_2}，实际 {get_res}"
