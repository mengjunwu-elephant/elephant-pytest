...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "is_power_on")
...
def test_is_power_on3(device, case):
    title = case["title"]
    ...
    with allure.step("调用 is_power_on 接口"):
        response = device.m.is_power_on()
...
def test_is_power_on2(device, case):
    title = case["title"]
    expected = case["expect_data"]
