...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_fresh_mode")
...
def test_get_fresh_mode1(device, case):
    ...
    with allure.step("调用 get_fresh_mode 接口"):
        response = device.m.get_fresh_mode()
...
def test_get_fresh_mode2(device, case):
    title = case["title"]
    expected = case["expect_data"]
