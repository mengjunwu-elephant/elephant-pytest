...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "get_encoder")
...
def test_get_encoder1(device, case):
    ...
    with allure.step("调用 get_encoder 接口"):
        response = device.m.get_encoder(joint)
    ...
    with allure.step("断言接口返回结果"):
        ...
        assert_almost_equal(response, expected, tol=22) #tol代表允许的误差值
...
def test_get_encoder2(device, case):
    title = case["title"]
    joint = case["joint"]
