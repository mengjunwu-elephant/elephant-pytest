...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot320Base.TEST_DATA_FILE, "set_encoders")
...
def test_set_encoders1(device, case):
    title = case["title"]
    expected = case["expect_data"]
    encoders = case["encoders"]
    speed = case["speed"]

    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')
    logger.debug(f'encoders:{case["encoders"]}')
    logger.debug(f'speed:{case["speed"]}')

    with allure.step(f"调用 set_encoder 接口, encoders = {case['encoders']}, speed = {case['speed']}"):
        set_res = device.m.set_encoders(eval(encoders), speed)
        time.sleep(4)
        get_res = device.m.get_encoders()
        logger.debug(f"set_res返回:{set_res},get_res返回:{get_res}")

    with allure.step("机械臂回零"):
        device.go_zero()

    with allure.step("断言返回值类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误,应为{type(expected)},实际为 {type(set_res)}"

    with allure.step("断言 set_encoders 返回值"):
    ...
    with allure.step("断言 get_encoders 返回值"):
        allure.attach(str(eval(encoders)), name="get接口期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="get接口实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, eval(encoders), tol=22) #tol代表允许的误差值
...
def test_set_encoders2(device, case):
    title = case["title"]
    encoders = case["encoders"]
    speed = case["speed"]
    ...
    with pytest.raises(MyCobot320DataException, match=".*"):
        device.m.set_encoders(eval(encoders), speed)
