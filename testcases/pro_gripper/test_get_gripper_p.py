...
# 获取测试数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "get_gripper_p")
...
@allure.feature("获取夹爪 P 参数")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_gripper_p(device, case):
    title = case["title"]
    ...
    with allure.step("调用接口 get_gripper_P 获取 P 值"):
        response = device.m.get_gripper_P()
