...
def device():
    ...
    yield dev
    dev.m.set_gripper_output(0)  # 恢复默认状态
    dev.m.close()
    logger.info("环境清理完成，接口测试结束")


@allure.feature("设置夹爪输出")
@allure.story("正常值测试")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_output_normal(device, case):
    title = case['title']
    ...
    with allure.step("发送设置请求"):
        response = device.m.set_gripper_output(case["parameter"])
...
def test_set_output_exception(device, case):
    title = case['title']
    logger.info(f"》》》》》用例【{title}】开始测试《《《《《")
