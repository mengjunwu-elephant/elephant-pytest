...
# 获取测试数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "get_to_set_speed")
...
@allure.feature("获取设置速度")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_to_set_speed(device, case):
    title = case["title"]
    ...
    with allure.step("调用接口 get_gripper_speed 获取速度设置"):
        response = device.m.get_gripper_speed()
