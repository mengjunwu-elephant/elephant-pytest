...
# 获取测试数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "set_gripper_torque")
...
def device():
    ...
    yield dev
    dev.m.set_gripper_torque(100)  # 恢复默认值
    dev.m.close()
...
def test_set_gripper_torque_normal(device, case):
    title = case["title"]
    ...
    with allure.step("设置扭矩值"):
        set_res = device.m.set_gripper_torque(case["parameter"])
        logger.debug(f"设置返回值：{set_res}")

    with allure.step("获取当前扭矩值"):
        get_res = device.m.get_gripper_torque()
...
def test_set_gripper_torque_exception(device, case):
    title = case["title"]
    logger.info(f"》》》》》用例【{title}】开始测试《《《《《")
    ...
    with allure.step("尝试设置异常值，期望抛出 ValueError"):
        with pytest.raises(ValueError, match="value错误"):
            device.m.set_gripper_torque(case["parameter"])
