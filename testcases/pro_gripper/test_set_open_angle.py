...
# 获取测试数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "set_open_angle")
...
def device():
    ...
    yield dev
    dev.m.set_gripper_io_open_value(100)  # 恢复默认张开角度
    dev.m.close()
...
def test_set_open_angle_normal(device, case):
    ...
    with allure.step("发送设置请求"):
        response = device.m.set_gripper_io_open_value(case["parameter"])
...
def test_set_open_angle_exception(device, case):
    ...
    with allure.step("尝试设置异常值并捕获 ValueError"):
        with pytest.raises(ValueError, match="value错误"):
            device.m.set_gripper_io_open_value(case["parameter"])
