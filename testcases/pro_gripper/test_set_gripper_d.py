...
# 获取测试数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "set_gripper_d")
...
def device():
    ...
    yield dev
    dev.m.set_gripper_D(120)  # 恢复默认值
    dev.m.close()
    logger.info("环境清理完成，接口测试结束")


@allure.feature("设置夹爪D参数")
@allure.story("正常用例")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_set_gripper_d_normal(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_gripper_D 设置 D 值"):
        set_res = device.m.set_gripper_D(case["parameter"])
        logger.debug(f"设置返回值：{set_res}")

    with allure.step("调用 get_gripper_D 获取当前 D 值"):
        get_res = device.m.get_gripper_D()
        logger.debug(f"获取返回值：{get_res}")

    with allure.step("断言设置返回值类型为 int"):
        assert isinstance(set_res, int), f"返回类型错误，期望 int，实际为 {type(set_res)}"
        logger.debug("请求类型断言成功")

    with allure.step("断言设置读取结果与期望值一致"):
...
def test_set_gripper_d_exception(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    ...
    with allure.step("断言设置 D 值非法时抛出 ValueError"):
        with pytest.raises(ValueError):
            device.m.set_gripper_D(case["parameter"])
