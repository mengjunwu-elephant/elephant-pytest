...
# 从Excel中提取数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "get_gripper_d")

@pytest.fixture(scope="module")
def device():
    dev = ProGripperBase()  # 实例化夹爪
    logger.info("初始化完成，接口测试开始")
    yield dev
    dev.m.close()
    logger.info("环境清理完成，接口测试结束")

@allure.feature("获取夹爪D值")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_gripper_d(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_gripper_D 接口"):
        response = device.m.get_gripper_D()
