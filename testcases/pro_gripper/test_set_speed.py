...
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "set_speed")
...
def device():
    ...
    yield dev
    dev.m.set_gripper_speed(100)  # 恢复默认速度
    dev.m.close()
...
def test_set_speed_normal(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_gripper_speed 设置速度"):
        response = device.m.set_gripper_speed(case["parameter"])
        logger.debug(f"设置返回值：{response}")
        allure.attach(str(response), name="设置返回值", attachment_type=allure.attachment_type.TEXT)
...
def test_set_speed_exception(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    ...
    with allure.step("断言设置速度非法时抛出 ValueError"):
        with pytest.raises(ValueError):
            device.m.set_gripper_speed(case["parameter"])
