...
# 从Excel中提取数据
cases = get_test_data_from_excel(ProGripperBase.TEST_DATA_FILE, "get_gripper_i")
...
@allure.feature("获取夹爪I值")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_gripper_i(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_gripper_I 接口"):
        response = device.m.get_gripper_I()
