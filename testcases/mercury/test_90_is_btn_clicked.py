...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryBase.TEST_DATA_FILE, "is_btn_clicked")
...
@allure.feature("末端led按钮是否按下")
@allure.story("正确末端led按钮是否按下")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_is_btn_clicked(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.is_btn_clicked()
