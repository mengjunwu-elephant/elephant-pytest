...
def test_get_atom_modify_version_power_on(device, case):
    title = case["title"]
    ...
    with allure.step("左右臂发送请求"):
        ...
        r_response = device.mr.get_atom_modify_version()
...
def test_get_atom_modify_version_power_on_only(device, case):
...
def test_get_atom_modify_version_power_off(device, case):
    device.power_off()
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"test_api: {case['api']}")
