...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryBase.TEST_DATA_FILE, "release_servo")
...
@allure.story("正确设置单个电机掉电")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_release_servo1(device, case):
    title = case["title"]
    ...
    res = input(f'{case["joint"]}关节有无放松,输入1未放松,其他放松')

    with allure.step("断言机械臂是否放松"):
        assert res != '1', f"机械臂未放松"
