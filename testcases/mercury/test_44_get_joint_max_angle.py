...
def test_get_joint_max_angle_normal(device, case):
    ...
    with allure.step("获取机械臂最大角度"):
        response = device.mc.get_joint_max_angle(joint_id)
        logger.info(f"机械臂最大角度：{response}")

    with allure.step("断言返回值正确"):
        assert isinstance(response, float), f"机械臂返回类型错误：{type(response)}"
        assert response == l_expect, f"机械臂最大角断言失败：期望={l_expect}, 实际={response}"
...
def test_get_joint_max_angle_exception(device, case):
    ...
    with allure.step("断言非法 ID 抛出 MercuryDataException"):
        with pytest.raises(MercuryDataException) as exc_info:
            device.mc.get_joint_max_angle(joint_id)

    logger.info(f"✅ 用例【{case['title']}】触发了预期异常: {exc_info.value}")
    logger.info(f"》》》用例【{title}】测试完成《《《")

@allure.feature("获取关节最大角度")
@allure.story("仅上电调用 get_joint_max_angle 接口")
...
@allure.feature("获取关节最大角度")
@allure.story("下电调用 get_joint_max_angle 接口")
