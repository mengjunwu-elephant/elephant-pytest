...
def test_save_or_not(device, case):
    ...
    with allure.step("读取最大速度配置参数"):
        res = device.mc.get_max_acc(case['mode'])
...
@allure.feature("设置最大加速度")
@allure.story("仅上电调用 set_max_acc 接口")
...
def test_power_on_only(device, case):
    ...
    with allure.step("发送 set_max_acc 指令（机械臂）"):
...
@allure.feature("设置最大加速度")
@allure.story("下电调用 set_max_acc 接口")
