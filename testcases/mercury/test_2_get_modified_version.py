...
# 获取全部测试用例数据
cases = get_test_data_from_excel(MercuryBase.TEST_DATA_FILE, "get_modified_version")
...
@allure.feature("获取固件修改版本号")
@allure.story("上电 - 获取固件修改版本号")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "power_on"], ids=lambda c: c["title"])
def test_get_modified_version_power_on(device, case):
    title = case["title"]
    ...
    with allure.step("左右臂发送请求"):
        l_response = device.ml.get_modified_version()
        r_response = device.mr.get_modified_version()
...
def test_get_modified_version_power_on_only(device, case):
    ...
    with allure.step("右臂断言响应结果"):
        ...
        assert l_response == case['l_expect_data'], f"左臂断言失败"
        assert r_response == case['r_expect_data'], f"右臂断言失败"
