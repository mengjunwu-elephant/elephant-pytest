...
def test_set_solution_angles_normal(device, case):
    ...
    with allure.step("发送 set_solution_angles 指令"):
        response = device.mc.set_solution_angles(case["parameter"], case["speed"])

    with allure.step("断言返回类型为 int"):
        assert isinstance(response, int), f"机械臂返回类型应为 int，实际为 {type(response)}"

    with allure.step("断言返回值是否正确"):
        assert response == case["l_expect_data"], f"机械臂返回值不匹配，期望：{case['l_expect_data']}，实际：{response}"
...
def test_set_solution_angles_exception(device, case):
    ...
    with allure.step("发送非法参数，断言抛出异常"):
        with pytest.raises(MercuryDataException) as exc_info:
            device.mc.set_solution_angles(case["parameter"], case["speed"])

    logger.info(f"✅ 用例【{case['title']}】触发了预期异常: {exc_info.value}")
    logger.info(f"》》》用例【{title}】测试完成《《《")

@allure.feature("解算角度设置")
@allure.story("仅上电调用 set_solution_angles 接口")
...
def test_power_on_only(device, case):
    ...
    with allure.step("发送 set_solution_angles 指令（机械臂）"):
        response = device.mc.set_solution_angles(case["parameter"], case["speed"])
...
@allure.feature("解算角度设置")
@allure.story("下电调用 set_solution_angles 接口")
