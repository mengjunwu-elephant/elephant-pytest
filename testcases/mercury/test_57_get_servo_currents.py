...
# 从Excel加载用例
cases = get_test_data_from_excel(MercuryBase.TEST_DATA_FILE, "get_servo_currents")
...
@allure.feature("获取舵机电流")
@allure.story("正常用例 - 获取机械臂舵机电流")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "normal"], ids=lambda c: c["title"])
def test_get_servo_currents_normal(device, case):
    title = case["title"]
    logger.info(f"》》》用例【{title}】开始测试《《《")
    logger.debug(f"测试API: {case['api']}")
    logger.debug(f"测试参数: {case['parameter']}")

    with allure.step("获取机械臂舵机电流"):
...
@allure.feature("获取舵机电流")
@allure.story("仅上电调用 get_servo_currents 接口")
...
@allure.feature("获取舵机电流")
@allure.story("下电调用 get_servo_currents 接口")
