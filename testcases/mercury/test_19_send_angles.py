...
def test_send_angles_normal(device, case):
    ...
    with allure.step("发送 send_angles 指令到机械臂"):
        response = device.mc.send_angles(angles, speed,_async=True)
        device.wait()
        device.wait()
    ...
    with allure.step('断言 get_angles 接口返回值是否匹配预期'):
        ...
        allure.attach(str(angles), name="机械臂期望", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res,angles,tol=1,name='机械臂全关节运动'), f"机械臂响应不一致，期望: {angles}，实际: {get_res}"
...
def test_send_angles_exception(device, case):
    ...
    with allure.step("尝试发送非法角度并期望抛出 MercuryDataException"):
        with pytest.raises(MercuryDataException, match=".*") as exc_info:
            device.mc.send_angles(angles, speed)
