...
def test_get_robot_status_power_on(device, case):
    ...
    with allure.step("断言状态值正确"):
        assert l_response == eval(case["l_expect_data"]), f"左臂状态错误，期望：{case['l_expect_data']}，实际：{l_response}"
        assert r_response == eval(case["r_expect_data"]), f"右臂状态错误，期望：{case['r_expect_data']}，实际：{r_response}"
