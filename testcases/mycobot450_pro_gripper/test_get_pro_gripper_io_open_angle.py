...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "get_pro_gripper_io_open_angle")

@allure.feature("获取夹爪张开角度")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_open_angle(device, case):
    title = case["title"]
    ...
    with allure.step("调用接口 get_pro_gripper_io_open_angle 获取张开角度"):
        response = device.mc.get_pro_gripper_io_open_angle()
