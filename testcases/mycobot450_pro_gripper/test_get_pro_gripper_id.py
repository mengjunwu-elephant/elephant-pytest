...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "get_pro_gripper_id")

@allure.feature("获取夹爪ID")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_pro_gripper_id(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_pro_gripper_id 接口获取固件版本号"):
        response = device.mc.get_pro_gripper_id()
