...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "set_protection_current")
...
def device():
    ...
    yield dev
    dev.mc.set_pro_gripper_protection_current(300)  # 恢复默认保护电流
    dev.mc.close()
...
def test_set_protection_current_normal(device, case):
    ...
    with allure.step("发送设置保护电流请求"):
        set_res = device.mc.set_pro_gripper_protection_current(case["parameter"])
        logger.debug(f"设置返回值: {set_res}")

    with allure.step("读取保护电流当前值"):
        get_res = device.mc.get_pro_gripper_protection_current()
...
def test_set_protection_current_exception(device, case):
    ...
    with allure.step("尝试设置异常值并捕获 MyCobotPro450DataException"):
        with pytest.raises(MyCobotPro450DataException):
            device.mc.set_pro_gripper_protection_current(case["parameter"])
