...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_io_close_angle")
...
def test_set_close_angle_out_of_limit(device, case):
    ...
    with allure.step(f"调用 set_pro_gripper_io_close_angle({case['parameter']}) 并期望抛出 MyCobotPro450DataException"):
        with pytest.raises(MyCobotPro450DataException, match=".*"):
