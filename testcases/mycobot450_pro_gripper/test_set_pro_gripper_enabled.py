...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_enabled")
...
def test_set_pro_gripper_enabled_normal(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_pro_gripper_enabled 设置使能状态"):
        response = device.mc.set_pro_gripper_enabled(case["parameter"])
...
def test_set_pro_gripper_enabled_exception(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
