import pytest
import allure
from common1.test_data_handler import get_test_data_from_excel
from common1 import logger
from settings import Mycobot450Base

cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "get_firmware_version")

pytestmark = pytest.mark.smoke

@allure.feature("获取固件版本号")
@allure.story("正常用例")
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_pro_gripper_firmware_version(device, case):
    title = case["title"]
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')

    with allure.step("打印测试参数信息"):
        logger.debug(f'test_api: {case["api"]}')
        logger.debug(f'test_parameters: {case.get("parameters", "")}')

    with allure.step("调用 get_pro_gripper_firmware_version 接口获取固件版本号"):
        response = device.mc.get_pro_gripper_firmware_version()
        logger.debug(f"接口返回值：{response}")

    with allure.step("断言返回值类型为 float"):
        assert isinstance(response, float), f"返回类型错误，期望 float，实际为 {type(response)}"
        logger.debug("请求类型断言成功")

    with allure.step("断言返回结果与期望一致"):
        allure.attach(str(case["expect_data"]), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(response), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert response == case["expect_data"], f"期望：{case['expect_data']}，实际：{response}"

    logger.info(f'✅ 用例【{title}】测试通过')
    logger.info(f'》》》》》用例【{title}】测试完成《《《《《')
