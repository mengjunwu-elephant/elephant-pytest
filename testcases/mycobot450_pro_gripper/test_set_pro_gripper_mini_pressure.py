...
cases = get_test_data_from_excel(Mycobot450Base.PRO_GRIPPER_TEST_DATA_FILE, "set_pro_gripper_mini_pressure")
...
def device():
    ...
    yield dev
    dev.mc.set_pro_gripper_mini_pressure(100)
    dev.mc.close()
...
def test_set_gripper_min_pressure_normal(device, case):
    ...
    with allure.step("发送设置最小启动力指令"):
        set_res = device.mc.set_pro_gripper_mini_pressure(case["parameter"])
        logger.debug(f"设置返回值: {set_res}")

    with allure.step("读取当前最小启动力值"):
        get_res = device.mc.get_pro_gripper_mini_pressure()
