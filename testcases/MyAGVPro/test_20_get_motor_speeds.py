...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "get_motor_speeds")
...
def test_get_motor_speeds1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_motor_speeds 接口"):
        response = device.mc.get_motor_speeds()
    ...
    with allure.step("断言读取接口返回结果"):
        ...
        assert_almost_equal(response, expected, 44,'电机转速'), f"用例【{title}】断言失败，期望 {expected},实际 {response}"
...
def test_get_motor_speeds2(device, case):
    title = case["title"]
