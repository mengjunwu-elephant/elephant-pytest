...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "get_wifi_ip_port")


@allure.feature("读取WiFi-ip、端口号")
@allure.story("读取WiFi-ip、端口号（上电）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "power_on"], ids=lambda c: c["title"])
def test_get_wifi_ip_port1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_wifi_ip_port 接口"):
        response = device.mc.get_wifi_ip_port()
