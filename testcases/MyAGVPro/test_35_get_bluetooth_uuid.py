...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "get_bluetooth_uuid")


@allure.feature("读取蓝牙名称、uuid")
@allure.story("读取蓝牙名称、uuid（上电）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "power_on"], ids=lambda c: c["title"])
def test_get_bluetooth_uuid1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 get_bluetooth_uuid 接口"):
        response = device.mc.get_bluetooth_uuid()
