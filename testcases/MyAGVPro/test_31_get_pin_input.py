...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "get_pin_input")

input(f'即将开始读取输入引脚状态, 在连接继电器后, 回车继续测试')

@allure.feature("读取输入引脚状态")
@allure.story("读取输入引脚状态（上电）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_pin_input1(device, case):
    title = case["title"]
...
def test_get_pin_input2(device, case):
    ...
    with allure.step("调用 get_pin_input 接口"):
        response = device.mc.get_pin_input(case['pin'])
