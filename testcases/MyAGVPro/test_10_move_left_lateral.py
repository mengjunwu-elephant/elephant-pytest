...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "move_left_lateral")
...
def test_move_left_lateral1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 move_left_lateral 接口"):
        response = device.mc.move_left_lateral(case['parameters'])
...
def test_move_left_lateral2(device, case):
    title = case["title"]
    expected = case["expect_data"]
...
def test_move_left_lateral3(device, case):
    title = case["title"]
