...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "turn_right")


@allure.feature("小车右旋")
@allure.story("小车右旋（上电）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "power_on"], ids=lambda c: c["title"])
def test_turn_right1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 turn_right 接口"):
        response = device.mc.turn_right(case['parameters'])
        logger.debug(f"接口返回：{response}")

    res = input(f'查看小车是否右旋运动, 是回车, 不是输入1\n')
...
@allure.feature("小车右旋")
@allure.story("小车右旋（参数超限）")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "exception"], ids=lambda c: c["title"])
def test_turn_right3(device, case):
    title = case["title"]
