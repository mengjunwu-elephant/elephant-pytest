...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "get_motor_enable_status")
...
def test_get_motor_enable_status1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 set_motor_enable 接口"):
        ...
        logger.debug(f"set_motor_enable接口返回：{response}")
        sleep(0.1)

    with allure.step("调用 get_motor_enable_status 接口"):
        response = device.mc.get_motor_enable_status()
        logger.debug(f"get_motor_enable_status接口返回：{response}")
...
def test_get_motor_enable_status2(device, case):
    title = case["title"]
