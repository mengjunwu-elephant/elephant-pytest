...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MyAGVProBase.TEST_DATA_FILE, "move_backward")


@allure.feature("小车向后")
@allure.story("小车向后（上电）")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "power_on"], ids=lambda c: c["title"])
def test_move_backward1(device, case):
    title = case["title"]
    ...
    with allure.step("调用 move_backward 接口"):
        response = device.mc.move_backward(case['parameters'])
        logger.debug(f"接口返回：{response}")

    res = input(f'查看小车是否向后运动, 是回车, 不是输入1\n')
...
@allure.feature("小车向后")
@allure.story("小车向后（参数超限）")
@pytest.mark.parametrize("case", [c for c in cases if c.get("test_type") == "exception"], ids=lambda c: c["title"])
def test_move_backward3(device, case):
    title = case["title"]
