...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "angle_correct_solution")


@allure.feature("角度正解")
@allure.story("正确计算角度正解")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_angle_correct_solution0(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.angle_correct_solution(angles)
    ...
    with allure.step("断言接口返回结果"):
        ...
        assert_almost_equal(response, expected, tol, '角度正解')
