...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "get_end_io_state")


@allure.feature("末端IO状态")
@allure.story("读取单个末端IO引脚状态")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_end_io_state(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_end_io_state(case["pin_no"])
...
def test_get_end_io_state_exception(device, case):
    title = case["title"]
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException,引脚为{case['pin_no']}"):
        with pytest.raises(ultraArmP1DataException) as exc:
            device.mc.get_end_io_state(case["pin_no"])
