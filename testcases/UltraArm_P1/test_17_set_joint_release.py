...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "set_joint_release")
...
@allure.feature("设置机械臂掉使能")
@allure.story("正确获取机械臂掉使能")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_joint_release(device, case):
    title = case["title"]
    ...
    with allure.step(f'调用 {case["api"]} 接口'):
        response = device.mc.set_joint_enable()
