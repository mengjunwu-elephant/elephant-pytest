...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "jog_coord")
...
def test_jog_coord_exception(device, case):
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException,笛卡尔坐标系为{case['axis']},方向为{case['position']}, 速度为{case['speed']}"):
