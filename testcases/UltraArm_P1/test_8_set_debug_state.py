...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_debug_state")
...
@allure.feature("设置日志模式")
@allure.story("正确设置日志模式")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_debug_state1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_debug_state(case["mode"])
        logger.debug(f"接口返回：{set_res}")

    with allure.step(f'调用 get_debug_state 接口'):
        get_res = device.mc.get_debug_state()
...
def test_set_fresh_mode_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,模式为{case['mode']}"):
