...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "get_base_io_state")


@allure.feature("底座IO状态")
@allure.story("读取单个底座IO引脚状态")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_base_io_state(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_base_io_state(case["pin_no"])
...
def test_get_base_io_state_exception(device, case):
    title = case["title"]
