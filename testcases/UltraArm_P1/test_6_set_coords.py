...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "set_coords")
...
@allure.story("正确设置全坐标")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_coords0(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_coords(eval(case["coords"]),case["speed"])
    ...
    with allure.step("断言 get_coords 返回值"):
        ...
        assert_almost_equal(get_res, eval(case["coords"]), 0.2,'插补模式设置全坐标'), f"用例【{title}】断言失败，期望 {case['coords']},实际 {get_res}"
...
def test_set_coords_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 UltraArmDataException,角度为{case['coords']}, 速度为{case['speed']}"):
