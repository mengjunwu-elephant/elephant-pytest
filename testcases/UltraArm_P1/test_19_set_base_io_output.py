...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(UltraArmP1Base.TEST_DATA_FILE, "set_base_io_output")
...
def test_set_base_io_output1(device, case):
    ...
    logger.debug(f'signal:{case["pin_signal"]}')

    with allure.step(f"调用 {case['api']} 接口"):
        time.sleep(1)
        set_res = device.mc.set_base_io_output(case["pin_no"],case["pin_signal"],case["state"])
        logger.debug(f"接口返回：{set_res}")

    with allure.step('读取底座IO状态'):
        get_res = device.mc.get_base_io_state(case["pin_no"])
    ...
    with allure.step("断言底座IO状态"):
        ...
        assert case['current_signal'] == get_res, f"用例【{title}】断言失败，期望 {case['current_signal']},实际 {get_res}"
...
def test_set_base_io_output_exception(device, case):
    ...
    with allure.step(f"断言抛出 ultraArmP1DataException,引脚为{case['pin_no']}，状态为{case['state']}，信号为{case['pin_signal']}"):
        with pytest.raises(ultraArmP1DataException) as exc:
            device.mc.set_base_io_output(case['pin_no'],case['pin_signal'],case['state'])
