...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "set_motor_enabled")
...
def test_set_motor_enabled_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,模式为{case['mode']}"):
        with pytest.raises(MyCobotPro450DataException):
            device.mc.set_motor_enabled(case['joint'],case['mode'])
