...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(Mycobot450Base.TEST_DATA_FILE, "jog_increment_angle")
...
@allure.story("插补模式设置jog_increment_angle")
...
def test_jog_increment_angle0(device, case):
    ...
    with allure.step("断言 get_angle 返回值"):
        ...
        assert_almost_equal(get_res, case["increment"], 1,'插补模式设置jog_increment_angle'), f"用例【{title}】断言失败，期望 {case['increment']},实际 {get_res}"
...
@allure.feature("关节步进模式")
@allure.story("刷新模式设置jog_increment_angle")
...
def test_jog_increment_angle1(device, case):
    ...
    with allure.step("断言 get_angle 返回值"):
        allure.attach(str(case["increment"]), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, case["increment"], 1,'刷新模式设置jog_increment_angle'), f"用例【{title}】断言失败，期望 {case['increment']},实际 {get_res}"
...
def test_jog_increment_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 Mycobot450Exception,关节为{case['joint']},增量为{case['increment']}, 速度为{case['speed']}"):
