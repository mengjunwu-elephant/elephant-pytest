...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "set_vr_mode")
...
@allure.feature("设置VR模式")
@allure.story("正确设置VR模式")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_vr_mode1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_vr_mode(case["parameter"])
        time.sleep(1)
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 get_vr_mode 接口'):
        get_res = device.mc.get_vr_mode()
...
def test_set_vr_mode_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,参数为{case['parameter']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.set_vr_mode(case['parameter'])
