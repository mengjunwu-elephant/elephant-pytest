...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "set_base_io_output")
...
def test_set_base_io_output1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        time.sleep(1)
        set_res = device.mc.set_base_io_output(case["pin_no"],case["state"])
    ...
    with allure.step('读取底座IO状态'):
        get_res = device.mc.get_base_io_input(case["pin_no"])
...
def test_set_base_io_output_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,引脚为{case['pin_no']}，状态为{case['state']}"):
