...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "send_coord")
...
def test_send_coord_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,笛卡尔积坐标系为{case['axis']},坐标值为{case['coord']}, 速度为{case['speed']}"):
