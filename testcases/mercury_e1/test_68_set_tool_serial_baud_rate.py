...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "set_tool_serial_baud_rate")
...
def device():
    ...
    yield dev
    dev.mc.set_tool_serial_baud_rate(115200)    #恢复默认波特率
    dev.mc.close()
    logger.info("环境清理完成，接口测试结束")

@allure.feature("设置末端485波特率")
@allure.story("正确设置末端485波特率")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_set_tool_serial_baud_rate1(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_tool_serial_baud_rate(case["parameter"])
        time.sleep(1)
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 get_tool_config 接口'):
        get_res = device.mc.get_tool_config()[0]
...
def test_set_tool_serial_baud_rate_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,参数为{case['parameter']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.set_tool_serial_baud_rate(case['parameter'])
