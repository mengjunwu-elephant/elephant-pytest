...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "jog_angle")
...
def test_jog_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,关节为{case['joint']},方向为{case['position']}, 速度为{case['speed']}"):
