...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "solve_inv_kinematics")
...
def test_solve_inv_kinematics_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,目标坐标为{target_coords}，当前角度为{current_angles}"):
