...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "send_angles")
...
@allure.story("插补模式设置全角度")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_send_angles0(device, case):
    title = case["title"]
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.send_angles(eval(case["angles"]),case["speed"])
        device.wait()
    ...
    with allure.step("断言 get_angles 返回值"):
        ...
        assert_almost_equal(get_res, eval(case["angles"]), 1,'插补模式设置全角度'), f"用例【{title}】断言失败，期望 {case['angles']},实际 {get_res}"
...
def test_send_angles1(device, case):
    ...
    with allure.step("断言 get_angles 返回值"):
        allure.attach(str(case["angles"]), name="期望值", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(get_res), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert_almost_equal(get_res, eval(case["angles"]), 1,
                            '刷新模式设置全角度'), f"用例【{title}】断言失败，期望 {case['angles']},实际 {get_res}"
...
def test_send_angles_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,角度为{case['angles']}, 速度为{case['speed']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.send_angles(eval(case["angles"]), case["speed"])
