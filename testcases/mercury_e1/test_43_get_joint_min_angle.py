...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "get_joint_min_angle")
...
def test_get_joint_min_angle1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        response = device.mc.get_joint_min_angle(case["joint"])
...
def test_get_joint_min_angle_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,关节为{case['joint']}"):
        with pytest.raises(MercuryE1DataException) as exc:
            device.mc.get_joint_min_angle(case['joint'])
