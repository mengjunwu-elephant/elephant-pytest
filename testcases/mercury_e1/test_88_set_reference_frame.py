...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "set_reference_frame")
...
def test_set_reference_frame1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_reference_frame(case["parameter"])
        time.sleep(1)
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 get_reference_frame 接口'):
        get_res = device.mc.get_reference_frame()
...
def test_set_reference_frame_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,参数为{case['parameter']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.set_reference_frame(case['parameter'])
