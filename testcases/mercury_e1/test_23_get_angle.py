...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "get_angle")
...
@allure.feature("读取单关节角度")
@allure.story("（读取单关节角度")
@pytest.mark.parametrize("case", [c for c in cases if c["test_type"] == "normal"], ids=lambda c: c["title"])
def test_get_angle1(device, case):
    title = case["title"]
...
def test_get_angle_exception(device, case):
    title = case["title"]
    expected = case["expect_data"]
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,关节为{case['joint']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.get_angle(case["joint"])
