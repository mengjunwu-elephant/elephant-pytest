...
# 从 Excel 读取测试数据
cases = get_test_data_from_excel(MercuryE1Base.TEST_DATA_FILE, "set_movement_type")
...
@allure.feature("设置移动类型")
@allure.story("正确设置移动类型")
...
def test_set_movement_type1(device, case):
    ...
    with allure.step(f"调用 {case['api']} 接口"):
        set_res = device.mc.set_movement_type(case["parameter"])
        time.sleep(1)
        logger.debug(f"接口返回：{set_res}")

    with allure.step('调用 get_movement_type 接口'):
...
def test_set_movement_type_exception(device, case):
    ...
    with allure.step(f"断言抛出 MercuryE1DataException,参数为{case['parameter']}"):
        with pytest.raises(MercuryE1DataException):
            device.mc.set_movement_type(case['parameter'])
